...
df_AA<- mutate(df_AA,comp=num_carriers-AA)

linear_m<-lm(AA~comp,df_AA)
summary(linear_m)
summary(linear_m)$coefficients[2,1]
...
ggplot(data=df_AA,aes(x=comp,y=AA))+geom_point()+
...
#another way to plot: first create a data frame containing predicted outcome for four models, then plot this data frame. use gather funtion.
```
